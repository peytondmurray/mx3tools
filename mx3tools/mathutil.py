...
@nb.jit(nopython=True)
def Axy_t(m_t: np.ndarray, t: np.ndarray) -> np.ndarray:
    """Return in-plane domain wall "activity" as a function of time. The domain wall is expected to run along the
    y-direction. Forward finite difference is taken for derivative.
    ...
    m_t : np.ndarray
        ...
        with other dimensions as in theta() docstring.
    t : np.nadarray
        1-Dimensional array of the simulation time for each time step.

    Returns
    -------
    np.ndarray
        Axy as a function of time; shape (Nt-1, t.shape[0])
    """

    return np.sum(mag_xy_t(m_t[:-1])*np.diff(phi_t(m_t), n=1)*mask_dw_t(m_t[:-1]))


@nb.jit(nopython=True)
def Az_t(m_t: np.ndarray, t) -> np.ndarray:
    """Return out-of-plane domain wall "activity" as a function of time. The domain wall is expected to run along the
    y-direction. Forward finite difference is taken for derivative.
    ...
    return np.sum(np.diff(theta_t(m_t), n=1)*mask_dw_t(m_t[:-1]))


@nb.jit
def Axy_t_safe(m_list, band_size=20):
    Axy = np.zeros(len(m_list)-1)

    m_old = mask_dw(ovftools.unpack(m_list[0]), band_size=band_size)

    for i in tqdm.trange(Axy.shape[0]):
        m_new = mask_dw(ovftools.unpack(m_list[i+1]), band_size=band_size)
        Axy[i] = np.sum((phi(m_new) - phi(m_old))*mag_xy(m_new))
        m_old = m_new
...
@nb.jit
def Az_t_safe(m_list, band_size=20):
    Az = np.zeros(len(m_list)-1)

    m_old = mask_dw(ovftools.unpack(m_list[0]), band_size=band_size)

    for i in tqdm.trange(Az.shape[0]):
        m_new = mask_dw(ovftools.unpack(m_list[i+1]), band_size=band_size)
        Az[i] = np.sum(theta(m_new) - theta(m_old))
        m_old = m_new
